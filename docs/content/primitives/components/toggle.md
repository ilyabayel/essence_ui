...
<.demo theme="light" variant="primitive" component="toggle" css={primitive_css("toggle")}>
  <:heex>
...
<.highlights>
  ...
  <:item>Can be controlled via `pressed` and `on_pressed_change` LiveView events.</:item>
</.highlights>
...
<.anatomy>
  <:part name="Root">The toggle button.</:part>
</.anatomy>
...
<.data_attributes_table>
  <:row name="[data-state]" values={"on | off"}>Present when pressed or not.</:row>
  <:row name="[data-disabled]" values="Present when disabled">Present when the toggle is disabled.</:row>
</.data_attributes_table>

## Accessibility

Adheres to the [Button WAI-ARIA pattern](https://www.w3.org/WAI/ARIA/apg/patterns/button).

### Keyboard Interactions

<.keyboard_table>
  <:row keys="Space">Activates/deactivates the toggle.</:row>
  <:row keys="Enter">Activates/deactivates the toggle.</:row>
</.keyboard_table>
