...
<.anatomy>
  ...
  <:part name="Button">The `button` part.</:part>
  <:part name="Link">The `link` part.</:part>
  <:part name="Separator">The `separator` part.</:part>
  <:part name="Toggle Group">The `toggle_group` part.</:part>
  <:part name="Toggle Item">The `toggle_item` part.</:part>
</.anatomy>
...
See the live demo above and `storybook/primitives/toolbar.story.exs` for complete markup. Style with classes and `data-state` as described in the [styling](/primitives/docs/guides/styling) guide.

## Accessibility

Adheres to the [WAI-ARIA pattern](https://www.w3.org/WAI/ARIA/apg/patterns/toolbar).
