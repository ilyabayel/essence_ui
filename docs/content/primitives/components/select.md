...
<Select.root id="popper-select">
  <Select.trigger class="DemoSelectTrigger">
    <Select.value placeholder="Select a fruit…" />
    <Select.icon class="DemoSelectIcon">…</Select.icon>
  </Select.trigger>
  <Select.portal id="popper-select-portal">
    <Select.content id="popper-select-content" class="DemoSelectContent" data-position="popper">
...
```heex
<Select.item class="DemoSelectItem" value="carrot" disabled>
  <Select.item_text>Carrot</Select.item_text>
  <Select.item_indicator class="DemoSelectItemIndicator">…</Select.item_indicator>
</Select.item>
```

```css
.DemoSelectItem[data-disabled] {
  color: var(--mauve-8);
  pointer-events: none;
}
```
...
```heex
<Select.trigger class="DemoSelectTrigger" aria-label="Food">
...
```css
.DemoSelectTrigger[data-placeholder] {
  color: var(--mauve-9);
}
```
...
<Select.root id="country-select" value={@country} on_change="country_change">
  <Select.trigger class="DemoSelectTrigger" aria-label="Country">
  ...
  <Select.portal id="country-select-portal">
    <Select.content id="country-select-content" class="DemoSelectContent">
      <Select.viewport>
...
<Select.root id="labelled-select">
  <Select.trigger id="labelled-select-trigger" class="DemoSelectTrigger">…</Select.trigger>
  …
</Select.root>
```
...
def select(assigns) do
  ...
  <Select.root id={@id} value={@value} on_change={@on_change}>
    ...
    <Select.portal id={"#{@id}-portal"}>
      <Select.content id={"#{@id}-content"} class="DemoSelectContent">
        <Select.scroll_up_button class="DemoSelectScrollButton">…</Select.scroll_up_button>
        <Select.viewport>
          <Select.item :for={item <- @item} class="DemoSelectItem" value={item.value}>
            <Select.item_text>{render_slot(item)}</Select.item_text>
            <Select.item_indicator class="DemoSelectItemIndicator">…</Select.item_indicator>
          </Select.item>
        </Select.viewport>
        <Select.scroll_down_button class="DemoSelectScrollButton">…</Select.scroll_down_button>
      </Select.content>
    </Select.portal>
  </Select.root>
  """
