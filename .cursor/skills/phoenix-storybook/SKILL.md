...
description: >-
  ...
  themes, and examples — containers, essence-demo, theme data attrs, Variation
...
- Never mix theme attrs and `essence-demo` on the same container.
- Always `@moduledoc false`. Component stories: `layout: :one_column`.

## Primitives

Headless Radix-style demos. CSS is scoped under `.essence-demo[data-component="…"]` in `assets/css/primitives/`. Playwright often targets the same selector.
...
- Exception: **separator** uses `container: :iframe` and keeps `<div class="essence-demo" data-component="separator">` **inside** the template (iframe CSS isolation).
...
- Double `essence-demo` (on `container` **and** in template)
...
- [ ] Grep leftover `class="essence-demo"` in `storybook/primitives/*` templates — only **separator** should remain
- [ ] Story page loads in Storybook; demo CSS still applies (primitives: sandbox has `.essence-demo[data-component="…"]`)
