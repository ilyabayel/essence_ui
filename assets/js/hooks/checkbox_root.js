export const CheckboxRoot = {
  mounted() {
    ...
    this.input = this.el.nextElementSibling?.hasAttribute('data-essence-checkbox-input') 
    ...
    this._onClick = (event) => {
      ...
      const nextChecked = currentState === 'indeterminate' ? true : currentState !== 'checked';
    ...
    if (form) {
      this._onFormReset = () => {
        ...
        const initialChecked = initialState === 'indeterminate' ? 'indeterminate' : (initialState === 'checked');
  ...
  _setChecked(checked) {
    const newState = checked === 'indeterminate' ? 'indeterminate' : (checked ? 'checked' : 'unchecked');
